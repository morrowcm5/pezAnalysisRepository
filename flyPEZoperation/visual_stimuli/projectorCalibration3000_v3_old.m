...
function projectorCalibration3000_v3_old
...
switch pezName
    case 'pez3001' %stimulus computer no. 1
        ...
        sphereCircumference = 473.5;%in millimeters
...
brightDensitiesDest = fullfile(variablesDir,'brightDensities.mat');
...
[~,top10] = min(abs(topCpos-deg2rad(90-10)));
...
if calibrateOp == 5
    brighterIm = uint8(gainMatrixBrighter.*255);
    imwrite(brighterIm,imReadPath)
    judp('send',portNum,hostIP,int8(54))
end

if calibrateOp == 6
%     imwrite(whiteIm,imReadPath)
