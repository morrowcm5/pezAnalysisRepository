...
for iterFull = 1:trialsFull
    ...
    for posAzi = 1:numel(aziCpos)
        waitbar(posAzi/numel(aziCpos),hWaitI)
        scanName = rowNames(posAzi);
        disp(scanName)
        ...
        posOpCommand = 1;
        if iterFull > 1 || runMode == 2
            posOpCommand = 3;
        elseif posAzi > 1 %round(posAzi/2) == posAzi/2
            posOpCommand = 2;
        end
        switch posOpCommand
            case 1
                ...
                [photoDataCellLR,passB] = elevationDataAcquisition2servos_v3(posListLR,...
                    servoPos_azi(posAzi),5,minPos,maxPos);
        ...
        if runMode == 1
            ...
            [photoDataL,passB] = elevationDataAcquisition2servos_v3(servoPos,servoPos_azi(posAzi),...
                trialsEach,minPos,maxPos);
            if ~passB
                break
            end
            if iterFull == 1
                scanResults_rescan = zeros(trialsFull,numel(servoPos));
            else
            ...
            servoPos = servoPos_bot;%bottom, up
            [photoDataL,passB] = elevationDataAcquisition2servos_v3(servoPos,servoPos_azi(posAzi),...
                trialsEach,minPos,maxPos);
            if ~passB
                break
            ...
            if iterFull == 1
                scanResults_bot = zeros(trialsFull,numel(servoPos));
            else
            ...
            servoPos = servoPos_top;%bottom, up
            [photoDataL,passB] = elevationDataAcquisition2servos_v3(servoPos,servoPos_azi(posAzi),...
                trialsEach,minPos,maxPos);
            if ~passB
                break
            ...
            if iterFull == 1
                scanResults_top = zeros(trialsFull,numel(servoPos));
            else
        ...
        elseif runMode == 2
            ...
            if iterFull == 1
                scanResults_unbalanced = zeros(trialsFull,numel(servoPos));
            else
            ...
            if iterFull == 1
                scanResults_balanced = zeros(trialsFull,numel(servoPos));
            else
            ...
            save(scanStatsDest,'scanStats')
            figure,plot(scanResults_balanced')
        end
