...
%                 [missedFrames,whiteCt] = presentDiskStimulusLocal(deviceNoRef);
...
%nidaq control functions
    ...
    function trigDetect(~,event)
        ...
        switch recState
            case 1
                if round(max(recData)) == 5
                    diodeDataA = diodeDataB;
                    lightDataA = lightDataB;
                    recState = 2;
                end
            case 2
                if round(min(recData)) == 0
...
%     function [missedFrms,whiteCt] = presentDiskStimulusLocal(nDeviceNo)
%         hTimers = timerfindall;
%         timerStates = cell(1,size(hTimers,2));
%         for iT = 1:size(hTimers,2)
%             timerStates{iT} = hTimers(iT).Running;
%             if strcmp(timerStates{iT},'on'),disp('timer'),end
%         end
