...
%%%%% Message Panel
hMsgPnl = uipanel('Parent',hPanelsMain,'Position',[.66 .17 .33 .17],...
...
hTxtMsgA = uicontrol(hMsgPnl,'style','text','string',msgString,...
    ...
    'normalized','position',[.01 .6 .98 .35],'foregroundcolor',[0.6 0 0]);
hTxtMsgB = uicontrol(hMsgPnl,'style','text','string',msgString,...
    'fontsize',10,'backgroundcolor',backC,'units','normalized','fontunits',...
    'normalized','position',[.05 .01 .9 .55],'foregroundcolor',[0 0 .6],...
...
posOp = [Xops(1) Yops(headYops(1)+7) W*10 H*6
    ...
    Xops(29) Yops(headYops(3)+45) W*32 H*11
    ...
    Xops(1) Yops(headYops(3)+45) W*28 H*11
...
% pez text only
posOp = {[.55 .1 .4 .7],[.1 .1 .9 .7],[.1 .1 .9 .7],...
    [.1 .1 .9 .7],[.8 .1 .15 .7],[.05 .47 .3 .4],...
    [.05 .07 .3 .4],[.4 .5 .3 .4],[.4 .1 .3 .4],[.1 .45 .8 .45],...
    [.05 .7 .12 .1],[.45 .7 .27 .1],[.45 .56 .27 .1],[.45 .43 .27 .1],...
    [.05 .56 .4 .1],[.1 .675 .6 .1],[.05 .56 .15 .3],[.3 .56 .25 .3],...
    [.66 .56 .3 .3]};
hP = [14 1 2 3 4 8 8 8 8 10 12 12 12 12 12 9 5 5 5];
strOp = {'deg C','XX.X deg C','XX.X%','XXX% power',[num2str(lightInit) '%'],...
    'Shadow:','Gap:','Open: XXX%','Block: XXX%','Click reset to enable',...
    'l/v :','Elevation :','Azimuth :','Heading :','Length (ms)',...
    'Auto threshold:','Count:','Length(ms):','Power(%):'};
hName = {'target','temp','humid','fanpwr','IRlights',...
    'shadow','gap','openpos','closepos','flycount','loverv','ele',...
    'aziOff','aziFly','stimdur','autothresh','photoct','photodur','photoint'};
ctrlCt = numel(hP);
hPezReport = struct;
for iterG = 1:ctrlCt
...
posOp = {[.1 .1 .4 .8],[.2 .5 .1 .4],[.2 .1 .1 .4],[.2 .15 .6 .15],...
    [.1 .825 .8 .125],[.15 .85 .8 .125],[.15 .75 .8 .125],...
    [.15 .65 .8 .125],[.18 .72 .2 .1],[.76 .72 .2 .1],[.76 .58 .2 .1],...
    [.76 .45 .2 .1],[.05 .45 .3 .1],[.1 .15 .8 .1],[.1 .05 .8 .1],...
    [.65 .7 .25 .1],[.05 .1 .45 .3],[.05 .55 .45 .3],[.55 .55 .45 .3],...
    [.55 .1 .45 .3],[.05 .15 .6 .3],[.17 .6 .1 .34],[.51 .6 .12 .34],...
    [.85 .6 .1 .34]};
hP = [14 8 8 9 9 11 11 11 12 12 12 12 12 12 12 9 13 13 13 13 5 5 5 5];
styleOp = {'edit','edit','edit','checkbox','togglebutton','checkbox',...
    'checkbox','checkbox','edit','edit','edit','edit','edit',...
    'checkbox','checkbox','edit','checkbox','checkbox','checkbox',...
...
hName = {'target','shadow','gap','roi','showback','flydetect','runauto','annotate',...
    ...
    'autothresh','frameone','tenthrate','fullrate','restrict',...
    'photocouple','photoct','photodur','photoint'};
hPezRandom = struct;
for iterG = 1:ctrlCt
...
strOp = {'Calibrate','Sweep','Find Gates','Auto ROI','Manual ROI',...
    'Fine Focus','Reset','Initialize','Display','More','Less','Display'};
ctrlCt = numel(hP);
...
hPezButn.reset = uicontrol(hPezMode.parent,'Style','pushbutton',...
...
hTrigMode.parent = uibuttongroup('parent',hSubPnl(11),'position',...
    [.025 .405 .95 .22],'backgroundcolor',backC,'Title','Trigger on');
btnNames = {'Ready','Escaped'};
btnCt = numel(btnNames);
posOps = {[.05 .1 .45 .8],[.5 .1 .45 .8]};
...
btnNames = {'Searching','In View','Ready','Escaped'};
btnCt = numel(btnNames);
posOps = {[.05 .55 .35 .4],[.5 .55 .35 .4],[.5 .05 .35 .4],[.05 .05 .35 .4]};
...
% camera functions
    ...
    function updateGatePlot
        ...
        if ~isempty(htDataMCU)
            ...
            set(hPezReport.temp,'String',[num2str(t) ' deg C']);
...
%         end
        ...
        if (get(hPezRandom.couple,'Value') == get(hPezRandom.couple,'Max'))
            ...
            [nRet,nErrorCode] = PDC_TriggerIn(nDeviceNo);
            if (get(hPezRandom.photocouple,'Value') == get(hPezRandom.photocouple,'Max'))
                photoDisplayCall([],[])
            end
        end
%         messageFun('Trigger')
...
%             saveDir = 'C:\Users\cardlab\Documents\pez3000_testing';
        ...
        vidNameTenth = [runFolder,'_vid',sprintf('%04.0f',vid_count+1),'_tenthrate.mp4'];
        vidDestTenth = fullfile(runPath,vidNameTenth); %path to save next video
        vidNameFull = [runFolder,'_vid',sprintf('%04.0f',vid_count+1),'_fullrate.mp4'];
        vidDestFull = fullfile(runPath,vidNameFull); %path to save next video
        ...
        if get(hPezRandom.frameone,'Value') == 1
...
%         titleStr = ['Downloading: ' vidNameTenth];
...
%         vidObj = VideoWriter(vidDest,'Grayscale AVI');
        ...
        if get(hPezRandom.tenthrate,'Value') == 1
            titleStr = ['Downloading: ' vidNameTenth];
            ...
            vidObj = VideoWriter(vidDestTenth,'MPEG-4');
            open(vidObj)
        ...
        if get(hPezRandom.fullrate,'Value') == 1
            if get(hPezRandom.restrict,'Value') == 1
                
            end
            titleStr = ['Downloading: ' vidNameFull];
            hWait = waitbar(0,'1','Name',titleStr,...
                'CreateCancelBtn',...
                'setappdata(gcbf,''canceling'',1)');
            setappdata(hWait,'canceling',0)
            
            vidObj = VideoWriter(vidDestFull,'MPEG-4');
            open(vidObj)
...
%photostimulation
    function photoDisplayCall(~,~)
        lightsOn = round(str2double(hPezReport.photoint));
        lightsOn(lightsOn > 100) = 100;
        lightsOn(lightsOn < 0) = 0;
        lightsOff = 0;
        lightsDur = round(str2double(hPezReport.photoint))/1000;
        for iterP = 1:str2double(hPezReport.photoct)
            fwrite(sPez,sprintf('%s %u\r','L',lightsOn));
            pause(lightsDur)
            fwrite(sPez,sprintf('%s %u\r','L',lightsOff));
            pause(lightsDur)
        end
    end
    ...
    function displayVisStim(~,~)
        ...
        switch stimStrOps{get(visStimPop,'Value')}
            case 'Black disk on white background'
                ...
                missedFrames = presentDiskStimulus(stimTrigStruct,deviceNoRef);
...
% Find Gates
    function hFindButtonCallback(~,~)
        ...
        if caseVal == 3
            ...
            messageFun('Set gate to ''Closed'' state first');
        end
        set(hGateState.parent,'SelectedObject',hGateState.child(1))
    end
