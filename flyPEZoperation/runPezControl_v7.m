...
jTcpObj = [];
% declare global variable for TCP/IP connection to slave
    ...
    function initializeVisStim(~,~)
        ...
        if ~strcmp(initVersion,initState)
            switch initVersion
                case 'initA'
                    ...
                    disp('Initializing TCP/IP connection');
                    jTcpObj = jtcp('accept',21566,'timeout',5000);
                    jtcp('write',jTcpObj,1);
                    tic
                    while toc < 10
                        slave_msg = jtcp('read',jTcpObj);
                        if ~isempty(slave_msg)
                            break
                        end
                    end
                case 'initB'
        ...
        switch initVersion
            case 'initA'
                ...
                stim_data_matrix = [visParams.ellovervee,visParams.radiusbegin,visParams.radiusend];
                disp('Writing out the elleovervee, radiusbegin, radiusend');
                jtcp('write',jTcpObj,stim_data_matrix);
                   while toc < 10
                        slave_msg = jtcp('read',jTcpObj);
                        if ~isempty(slave_msg)
                            break
...
%                 [missedFrames,whiteCt] = presentDiskStimulus(stimTrigStruct,deviceNoRef);
                stim_data_matrix = [visParams.azimuth,visParams.elevation];
                [nRet,nErrorCode] = PDC_TriggerIn(nDeviceNo);
                jtcp('write',jTcpObj,stim_data_matrix);
                disp('Writing out the azi and ele');
                whiteCt = [];
                tic
                while toc < 10
                    whiteCt = jtcp('read',jTcpObj);
                    if ~isempty(whiteCt)
                        break
                    end
                end
                whiteCt
                whiteCt = str2double(whiteCt);
                size(whiteCt)
                class(whiteCt)
                %Dummy Values
                missedFrames = 0;
