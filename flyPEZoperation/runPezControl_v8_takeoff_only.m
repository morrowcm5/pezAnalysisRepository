function runPezControl_v8_takeoff_only
...
downloadStrCell = {'Save Cut Rate','Save Full Rate','Restricted full rate','No Download'};
...
tDet = timer('TimerFcn',@resetDetectFun,'ExecutionMode','fixedRate',...
    'Period',60,'StartDelay',30,'Name','tDet');
...
%             end
        ...
        elseif stageTab(1) == 1
            ...
            if stageTab(2) > 30 %number of frames which must be empty before reset
                timeTrkC = timeTrkB;
                if get(hTrigMode.parent,'selectedobject') == hTrigMode.child(2)
                    disp('trigger')
                    triggerCallback([],[])
                end
...
%         frmOps = (1:frmCount)';
    ...
    function downloadRecordingFun
        ...
        testC = strcmp(stimStrOps{get(visStimPop,'Value')},'Looming disk');
        testData = true;
        ...
        function closeDownload
            ...
            if ~discardBool
                ...
                vidStats2add = dataset({{vidName},'videoID'},{{memFrameInfo.m_nTrigger},'trigger_timestamp'},...
                    ...
                    {{visStimInfo},'visual_stimulus_info'},{{activationInfo},'photoactivation_info'},...
                    {{timeTrkC},'time_on_prism'},'ObsNames',{vidName});
