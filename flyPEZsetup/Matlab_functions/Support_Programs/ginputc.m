...
addParameter(p, 'LineWidth', 0.5 , @(x) validateattributes(x,{'numeric'}, {'scalar', 'positive'}));
addParameter(p, 'LineStyle', '-' , @(x) validatestring(x,{'-', '--', '-.', ':'}));
addParameter(p, 'ShowPoints', false, @(x) validateattributes(x,{'logical'}, {'scalar'}));
addParameter(p, 'ConnectPoints', true, @(x) validateattributes(x,{'logical'}, {'scalar'}));
addParameter(p, 'curr_axes', gca);
...
%hInvisibleAxes = axes('Units', 'normalized','Position', [0 0 1 1], 'XLim', [0 1],'YLim', [0 1], ...
...
if showpoints
    ...
    hPoints = line(nan, nan, ...
        'Parent', hAx,'HandleVisibility', 'off', ...
...
hCursor = line(nan, nan,'Parent', hAx, 'Color', color,'LineWidth', linewidth,'LineStyle', linestyle,'HandleVisibility', 'off','HitTest', 'off');
...
%--------------------------------------------------------------------------
    function mouseMoveFcn(varargin)
        ...
        cursorPt = get(hAx, 'CurrentPoint');
        
        set(hCursor,'XData', [0 1 nan cursorPt(1) cursorPt(1)], ...
