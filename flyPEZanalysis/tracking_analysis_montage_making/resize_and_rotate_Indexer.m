...
function [indexer_struct] = resize_and_rotate_Indexer(spoke_count,spoke_length,...
...
centerVec_c = (center:avg_test_c(1)+1);
centerVec_r = repmat(center,1,numel(centerVec_c));
centerVec = sub2ind([block_dim,block_dim],centerVec_r,centerVec_c);
cntr_ind = zeros(length(centerVec),spoke_count);
...
for j = 1:spoke_count
    ...
    cntr_ind(:,j) = box_rot(centerVec);
end
cntr_ind = unique(cntr_ind);
indexer_struct = struct('half_sml',pixel_last_of_small,'half_lrg',half_large,...
    ...
    length(pix_trg_ind_vec),'center_indexer',cntr_ind);
