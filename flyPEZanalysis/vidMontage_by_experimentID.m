function vidMontage_by_experimentID(totalComps,thisComp)
...
% [groupNames,refsFirst,refsFull] = unique(groupData.Properties.RowNames);
...
for iterE = exptRefBreaks(thisComp)%:exptRefBreaks(thisComp+1)
    ...
    switch length(activationProtocols)
        case 1
            protocolSums = sum(strcmp(automatedAnnotations.photoStimProtocol(masterList),activationProtocols{1}));
        case 2
            protocolSums = [sum(strcmp(automatedAnnotations.photoStimProtocol(masterList),activationProtocols{1})),...
                sum(strcmp(automatedAnnotations.photoStimProtocol(masterList),activationProtocols{2}))];
        case 3
            protocolSums = [sum(strcmp(automatedAnnotations.photoStimProtocol(masterList),activationProtocols{1})),...
                sum(strcmp(automatedAnnotations.photoStimProtocol(masterList),activationProtocols{2})),...
                sum(strcmp(automatedAnnotations.photoStimProtocol(masterList),activationProtocols{3}))];
    end
    masterList = masterList(strcmp(automatedAnnotations.photoStimProtocol(masterList),activationProtocols{protocolRef}));
    photoStimName = activationProtocols{protocolRef};
    vidsAvailable = protocolSums(protocolRef);
...
%    analyzer_name = 'pezAnalyzer3000_v11';
    ...
    for iterM = 1:vidCount
        ...
        yi = repmat(Y,10,1);
%         yi(end-3:end,:) = yi(end-3:end,:)+1;%makes it like interpolation but faster and no NaN
        yi = yi(:);
...
%         flyPosCell{iterM} = [flyPosX,flyPosY];
        
        analyzerRecord = load(analyzerPathList{iterM});
        dataname = fieldnames(analyzerRecord);
        analyzerRecord = analyzerRecord.(dataname{1});
        flyPosX{iterM} = analyzerRecord.top_points_and_thetas{1}(:,1);
        
        try
            vidObjSupplement{iterM} = VideoReader(vidSuppPath);
...
%     imshow(graphBase)
    ...
    prevFrames = cell(1,montFrmCt);
    cutRateOps = round(linspace(1,montFrmCt,round(montFrmCt/10)));%divisor determines rate.  1 is full rate
    for iterF = cutRateOps
        ...
        for iterM = 1:vidCount
            ...
            if readObjRef == 1
                usePrev = false;
                if iterF > 1
                    frmRef = frmBounds{iterM}(iterF-1);
                    if frameReferenceMesh{iterM}(1,(frmRef)) == frameRead
                        frmA = prevFrames{iterM};
                        usePrev = true;
                    end
                end
                if ~usePrev
