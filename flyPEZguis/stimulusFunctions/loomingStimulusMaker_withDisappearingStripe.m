...
wedgetime = 1500; %time wedge exists after trigger in ms
fadetime = 0; %time wedge fades over in ms
blanktime = 0; %time after fade and before loom in ms
...
%         '_' num2str(duration) 'ms_' strA];
    ...
    fileName = ['dwall_' num2str(wedgetime) '_' num2str(fadetime) '_' num2str(blanktime) '_w' num2str(stripe_width_deg) 'at' num2str(stripe_azimuth_center) '_h' num2str(stripe_height_deg) 'at'...
        elecent '_l_' initStimSizeStr 'to' num2str(finalStimSize),...
    ...
    stimTotalDuration = stimStartTime-stimEndTime+wedgetime+fadetime+blanktime;
...
wedgeframes = round(wedgetime/stimTimeStep); %frames per action
fadeframes = round(fadetime/stimTimeStep)+wedgeframes;
blankframes = round(blanktime/stimTimeStep)+fadeframes;

wedge = cell(blankframes,1);
for i = 1:blankframes
    if i <= wedgeframes
        wedge{i} = uint8(flipud(wall));
    elseif i<= fadeframes
        fadewedge = wall(:,:);
        for j = 1:length(ind1)
            fadewedge(ind1(j),ind2(j)) = 255*(i-wedgeframes)/(fadeframes-wedgeframes); %increases wedge foreground values from 0 to 1
        end
        wedge{i} = uint8(flipud(fadewedge));
    else
        wedge{i} = wedge{i-1};
    end
end

wedgeRemain = round((ceil(blankframes/3)-blankframes/3)*3);
if wedgeRemain == 1
    wedge{i+1} = wedge{i};
elseif wedgeRemain == 2
    wedge{i+1} = wedge{i};
    wedge{i+2} = wedge{i};
end

wedgeFrmCt = numel(wedge)/3;
imgWall = cell(wedgeFrmCt,1);
for iterPrep = 1:wedgeFrmCt %number of wedge frames
    imgWall{iterPrep} = cat(3,wedge{stimRefRGB(1)+(3*(iterPrep-1))},wedge{stimRefRGB(2)+(3*(iterPrep-1))},wedge{stimRefRGB(3)+(3*(iterPrep-1))});
end
 %wall = imfilter(wall,fspecial('average',[4 4]),'replicate');
