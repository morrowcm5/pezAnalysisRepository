        ...
        switch floor(opt.histOpt)
            ...
            case 1
                ...
                if opt.histOpt == 1.1
                    ...
                    [dummy,x] = hist(currentData); %#ok<ASGLU>
                    if length(x) == 1
                    ...
                    [xHist,yHist,u] = ksdensity(currentData,'kernel','normal','npoints',5000);
                    % take smaller kernel to avoid over-smoothing
                    if opt.divFactor ~= 1
                        [xHist,yHist] = ksdensity(currentData,'kernel','normal','width',(u/opt.divFactor),'npoints',5000);
%                        def = std(currentData)*length(currentData)^(-1/5);
%                        [xHist,yHist] = ksdensity(currentData,'kernel','normal','width',0.4*def,'npoints',5000);
                    end
                end
...
for iData = goodData'
    ...
    switch opt.xyOri
        ...
        case 'pat_special'
            hh(iData)= line([xArray(1,:),(xArray(2,end:-1:1))],[yArray(1,:),(yArray(2,end:-1:1))],'Parent',ah,'color',[0.5000 0.5000 1.0000],'linewidth',1.5);
    end
...
if opt.addSpread
    if isHistogram
        ...
        % add spread
        try
            if strcmp(opt.xyOri,'pat_special')
                sh = plotSpread(ah,data,'xValues',opt.xValues,'xyOri','normal');                
            else                    
